...
func GetCompanyProfile(db *sql.DB) (*CompanyProfile, error) {
	...
	err := db.QueryRow(
		...
		&defaultAcctID, &c.RecurringDescriptionTemplate, &c.UpdatedAt)
	if err != nil {
		return nil, fmt.Errorf("get company profile: %w", err)
	}
	if defaultAcctID != nil {
		c.DefaultRevenueAccountID = *defaultAcctID
	}
	return c, nil
}
...
func UpdateCompanyProfile(db *sql.DB, c *CompanyProfile) error {
	...
	if c.DefaultRevenueAccountID != 0 {
		defaultAcctID = &c.DefaultRevenueAccountID
	}
	_, err := db.Exec(
		`INSERT INTO company_profile
		 ...
		 ON CONFLICT(id) DO UPDATE SET
			...
			default_revenue_account_id = excluded.default_revenue_account_id,
			recurring_description_template = excluded.recurring_description_template,
			updated_at = datetime('now')`,
		c.Name, c.Tagline, c.Address, c.Phone, c.Email, c.NPWP,
		c.BankName, c.BankAccountNumber, c.BankAccountHolder, c.InvoiceFooter,
		defaultAcctID, c.RecurringDescriptionTemplate,
	)
	if err != nil {
